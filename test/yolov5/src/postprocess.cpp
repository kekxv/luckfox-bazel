...
#include <stdint.h>
#include "model_yolov5s-640-640.h"
#include "model_coco_80_labels_list.h"
...
char* readLineFromMemOptimized(const unsigned char* data, int size, int* offset, int* len)
{
  int start = *offset;
  ...
  int current = start;
  // 1. 先扫描找到换行符或者结尾
  while (current < size && data[current] != '\n')
  {
    current++;
  }

  // 2. 计算长度
  int line_length = current - start;
  ...
  if (line_length > 0)
  {
    memcpy(buffer, data + start, line_length);
  }
  buffer[line_length] = '\0';

  // 5. 更新偏移量 (如果是因为遇到 \n 停下的，offset 要跳过那个 \n)
  if (current < size && data[current] == '\n')
  {
    *offset = current + 1;
  }
  else
 ...
 * 读取所有行
 */
int readLinesFromMem(const unsigned char* data, int size, char* lines[], int max_line)
{
  int offset = 0; // 相当于文件指针，初始为 0
  char* s;
  int i = 0;
  int line_len = 0;

  // 循环调用，传入 offset 的地址
  while ((s = readLineFromMemOptimized(data, size, &offset, &line_len)) != NULL)
  {
    lines[i++] = s;
    if (i >= max_line)
      break;
...
int loadLabelName(char* label[])
{
  readLinesFromMem(
    (unsigned char*)model_coco_80_labels_list.data, model_coco_80_labels_list.size,
    label, OBJ_CLASS_NUM);
  return 0;
}
      ...
      for (int a = 0; a < anchor_per_branch; a++)
      {
        ...
        if (box_confidence >= thres_i8)
        {
          ...
          if (limit_score > threshold)
          {
            ...
            box_x = deqnt_affine_to_f32(ConvertNC1HWC2ToNHWC(input, dims, h, w, a * PROP_BOX_SIZE + 0), zp, scale) * 2.0
              - 0.5;
            box_y = deqnt_affine_to_f32(ConvertNC1HWC2ToNHWC(input, dims, h, w, a * PROP_BOX_SIZE + 1), zp, scale) * 2.0
              - 0.5;
            box_w = deqnt_affine_to_f32(ConvertNC1HWC2ToNHWC(input, dims, h, w, a * PROP_BOX_SIZE + 2), zp, scale) *
              2.0;
            box_h = deqnt_affine_to_f32(ConvertNC1HWC2ToNHWC(input, dims, h, w, a * PROP_BOX_SIZE + 3), zp, scale) *
              2.0;
            box_w = box_w * box_w;
            box_h = box_h * box_h;
