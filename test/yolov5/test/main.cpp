...
static unsigned char *load_image(const char *image_path, rknn_tensor_attr *input_attr, int *img_height, int *img_width)
{
  ...
  if (*img_width != req_width || *img_height != req_height)
  {
    ...
    // 1. 函数名改为 stbir_resize_uint8_linear
    // 2. 最后一个参数 channel 需要强转为 (stbir_pixel_layout)
    // 3. 判断条件改为 == NULL 表示失败 (旧版是 != 1)
    if (stbir_resize_uint8_linear(image_data, *img_width, *img_height, 0,
                                  image_resized, req_width, req_height, 0,
                                  (stbir_pixel_layout)channel) == NULL)
    {
...
int main(int argc, char *argv[])
{
  if (argc < 2)
  {
    printf("Usage:%s input_path [loop_count]\n", argv[0]);
    return -1;
  }
  ...
  int ret = rknn_init(&ctx, (void*)model_yolov5s_640_640.data, model_yolov5s_640_640.size, 0, NULL);
  if (ret < 0)
  {
